...
# 建立 Q-table：採用 dictionary 形式，key 為 state（轉成字串），value 為長度為 6 的 numpy 陣列
q_table = {}
...
for episode in range(EPISODES):
    ...
    while not done and steps < MAX_STEPS:
        ...
        state_key = str(state)
        q_table[state_key][action] = q_table[state_key][action] + ALPHA * (reward + GAMMA * np.max(get_q(next_state)) - q_table[state_key][action])
